...
export function ContentCard({
  ...
  return (
    ...
    >
      ...
      <div className={`${type === 'game' ? 'hidden md:flex' : 'flex'} bg-[#160026] border rounded-2xl overflow-hidden group-hover:border-[#F6CA56]/50 transition-all duration-300 group-hover:shadow-2xl group-hover:shadow-[#F6CA56]/10 h-full flex flex-col group-hover:-translate-y-2 relative ${type === 'game' ? 'border-[#F6CA56]/20' : 'border-[#351150]'}`}>

        {/* Image & Overlays */}
        <div className="relative h-64 md:h-56 overflow-hidden bg-[#0a0014] flex-shrink-0">
          {type === 'blog' && leftImage && rightImage ? (
            <div className="w-full h-full relative">
              <GameBlogHeader leftImage={leftImage} rightImage={rightImage} />
              <div className="absolute inset-0 bg-gradient-to-t from-[#160026] via-[#160026]/40 to-transparent pointer-events-none" />
            </div>
          ) : (
            <>
              <Image
          ...
          <button
            ...
            className="absolute bottom-4 right-4 z-20 w-10 h-10 bg-black/50 backdrop-blur-md rounded-full border border-white/10 flex items-center justify-center text-white/50 hover:text-[#F6CA56] hover:border-[#F6CA56]/50 transition-all group/tab opacity-0 group-hover:opacity-100"
            title="Open in new tab"
          >
            <ExternalLink className="w-4 h-4" />
          </button>

          {/* Score Badge (Balanced & Glowing) - Moved to Top Right */}
          {type === 'game' && rating && (
        ...
        <div className="p-6 flex flex-col flex-1">
          <div className="mb-4">
            <h3 className="text-2xl font-bold text-white mb-2 group-hover:text-[#F6CA56] transition-colors line-clamp-2 leading-tight font-heading tracking-tight">
              {title}
            </h3>
